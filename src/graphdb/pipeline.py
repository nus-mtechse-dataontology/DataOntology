...
from .response import (
    ...
    format_cities_in_country, format_country_info, format_country_weather,
...
class GraphDbPipeline:
    ...
    def __init__(self, fact_flight_info_dao: FactFlightInfoDAO):
        self._log = logging.getLogger("data_ontology")
        self._fact_flight_info_dao = fact_flight_info_dao
    
    def _eprint(*args, **kwargs):
        """Print to stderr so debug lines never reach the user-facing output."""
        print(*args, file=sys.stderr, **kwargs)
    
    def _friendly_columns(self, rows: list[dict]) -> list[dict]:
        """Return a copy of rows with internal column names replaced by friendly labels."""
        if not rows:
            return rows
        return [{self._COLUMN_RENAMES.get(k, k): v for k, v in row.items()} for row in rows]
    
    def _humanise_follow_up_dates(self, text: str) -> str:
        """Replace ISO dates with human-readable format and add month-level hint for date requests."""
        
        def _replace(m):
            try:
                y, mo, d = m.group(1), int(m.group(2)), int(m.group(3))
                return f"{self._MONTH_NAMES_FULL[mo - 1]} {d}, {y}"
            except Exception:
                return m.group(0)
    ...
    def run_once(
        ...
        if prefilled_plan is not None:
            ...
            try:
                ...
                self._eprint(f"  [LLM EXCEPTION] {e}")
        ...
        if phase in self.HYBRID_PHASES:
            ...
            if rows is not None:
                self._enrich_destination_names(rows)
        ...
        if phase in self.SQL_PHASES:
            ...
            if not rows:
                if params.get("trip_type") == "R":
                    ...
                    return query_plan
                if self._is_date_out_of_range(params):
                    print(
                ...
                if intent_name in ("cheapest_flight_on_route", "flights_on_date", "next_available_flight"):
                    ...
                    if transit_def:
                        ...
                        transit_rows = self._run_sql(transit_def, transit_params, "cheapest_transit_route")
                        if transit_rows:
            ...
            if rows and destination and has_triggers:
                ...
                if vp_template:
                    ...
                    try:
                        ...
                        print(format_table(self._friendly_columns(rows), intent_name))
                        return
                    ...
                    if visa_intent_name and passport_cc:
                        ...
                        if visa_template:
                            dest_cc = self._extract_country_code(graph, destination)
                            if dest_cc:
        ...
        if phase in self.SPARQL_ONLY_PHASES:
            ...
            if sparql_type == "construct":
                ...
                if visa_intent and passport_cc:
                    ...
                    if visa_template:
                        ...
                        dest_cc = self._extract_country_code(graph, params.get("destination_airport_code", ""))
                        if dest_cc:
                            visa_params = {
                                "passport_country_code": passport_cc,
                                "destination_country_code": dest_cc,
            ...
            params = self._inject_city_name(params)
            if intent_name == "destination_highlights":
                ...
                if code:
                    name_map = self._resolve_airport_names([code])
            ...
            elif intent_name == "country_weather_by_month":
                print(format_country_weather(rows, params))
                ...
                print(format_visa_check(rows,
                                        params) if intent_name == "visa_check_for_destination" else format_visa_list(
                    rows, params))
    ...
    def _is_date_out_of_range(self, params: dict) -> bool:
        ...
        for key in ("departure_date", "start_date", "end_date", "date"):
            ...
            if len(val) >= 7:
                try:
                    ...
                    if (year, month) > self._DATA_END:
                        return True
                except (ValueError, IndexError):
                    pass
        return False
    
    def _run_sql(self, intent_def: dict, params: dict, intent_name: str) -> list[dict] | None:
        ...
        try:
            rows = [
                dict(row)
                for row in self._fact_flight_info_dao.execute_raw_query(sql, bound)
            ]
        except Exception as e:
            err_str = str(e)
    ...
    def _resolve_airport_names(self, codes: list[str]) -> dict[str, str]:
        ...
        try:
            ...
            self._eprint(f"  [enrich] SPARQL lookup failed: {e}")
            return {}
    
    def _enrich_destination_names(self, rows: list[dict]) -> None:
        ...
        self._eprint(f"  [enrich] Resolving {len(codes)} destination code(s) via SPARQL...")
        name_map = self._resolve_airport_names(codes)
        if not name_map:
            self._eprint("  [enrich] WARNING: name resolution returned empty — codes will remain raw.")
            return
        resolved = 0
        for row in rows:
            ...
            if code and code in name_map:
                ...
                resolved += 1
        self._eprint(f"  [enrich] {resolved}/{len(codes)} code(s) resolved.")
    
    def _inject_city_name(self, params: dict) -> dict:
    ...
    def _extract_country_code(self, graph, airport_code: str) -> str | None:
    ...
    def main(self) -> None:
        ...
        while True:
            ...
            plan = self.run_once(question, semantics, intents_str, param_schema_str, history=history)
