...
CMD ["/bin/sh", "-c", "set -e; mkdir -p /tmp/project/resources /tmp/project/vault; cp /var/task/resources/config.toml /tmp/project/resources/config.toml; cp /var/task/vault/* /tmp/project/vault/; if [ -n \"$DB_USER\" ]; then printf \"%s\" \"$DB_USER\" > /tmp/project/vault/postgres.user; fi; if [ -n \"$DB_PASSWORD\" ]; then printf \"%s\" \"$DB_PASSWORD\" > /tmp/project/vault/postgres.password; fi; if [ -n \"$DB_HOST\" ]; then sed -i \"s|host = \\\"localhost\\\"|host = \\\"$DB_HOST\\\"|g\" /tmp/project/resources/config.toml; fi; if [ -n \"$DB_PORT\" ]; then sed -i \"s|port = 5432|port = $DB_PORT|g\" /tmp/project/resources/config.toml; fi; if [ -n \"$DB_NAME\" ]; then sed -i \"s|name = \\\"data_ontology\\\"|name = \\\"$DB_NAME\\\"|g\" /tmp/project/resources/config.toml; fi; export PROJECT_PATH=/tmp/project; python -c \"from src.main import DataOntology; d=DataOntology(); d._load_config(); d._init_app(); import uvicorn; uvicorn.run(d._app, host='0.0.0.0', port=8000)\""]